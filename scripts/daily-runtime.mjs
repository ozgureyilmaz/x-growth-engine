...
export const READ_TOOLS = ['x_search_tweets','x_get_profile','x_get_tweets','x_get_thread','x_get_replies'];
const count = z.number().int().positive();
const score = z.number().min(0).max(1);
export const ConfigSchema = z.object({
  schema_version:z.literal('1.0'), account:z.literal('nullquanty'),
  mode:z.enum(['FIXTURE_DRY_RUN','EXPERIMENTAL_LIVE_READ','REPLAY_REAL_DATA']), facts_path:z.string().optional(),
  ...
  publisher:z.object({enabled:z.literal(false),mode:z.literal('MANUAL_ONLY'),kill_switch:z.literal(true)}).strict(),
}).strict();
