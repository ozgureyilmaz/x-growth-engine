...
export function markdownBundle(bundle) {
  const lines=[`# X founder review — ${bundle.run_id}`,'',`Status: ${bundle.status||'UNKNOWN'} | Mode: ${bundle.mode} | model calls: ${bundle.model_calls||0}`,'','Publication is manual. Source completeness is recorded, not inferred.',''];
...
export async function executeDaily(options={}) {
  ...
  let now=options.now||new Date(),mode=options.mode||config.mode,maxDrafts=options.maxDrafts??config.intelligence.max_review_drafts;
  ...
  if(!['FIXTURE_DRY_RUN','EXPERIMENTAL_LIVE_READ','REPLAY_REAL_DATA'].includes(mode))throw new Error('UNSUPPORTED_MODE');
  ...
  try{return await withRunLock(root,async()=>{
    ...
    let source,health='UNKNOWN',calls=await store.modelCallsStarted(runId);
    ...
    try{
      if(maxDrafts>0&&mode!=='FIXTURE_DRY_RUN'){facts=await approvedFacts(config);if(!facts.length)throw new Error('MARX_FACT_REGISTRY_NOT_APPROVED');}
      ...
      if(mode==='REPLAY_REAL_DATA'){
        ...
        source=options.source||(mode==='FIXTURE_DRY_RUN'?new FixtureXSource(options.fixture||{search_results:[]}):new XActionsMcpSource({command:config.source.command,args:config.source.args,readTools:config.source.read_tools,callTimeoutMs:config.discovery.call_timeout_ms,delayMs:config.discovery.inter_call_delay_seconds*1000,signal}));
      ...
      let generated=[];
      if(contexts.length&&mode==='FIXTURE_DRY_RUN'){
      ...
      return{run_id:runId,status,mode,source_health:health,discovered:posts.size,drafts:selected.length,no_action:bundle.no_action_count,model_calls:calls,output_dir:out,publisher_enabled:false};
    }catch(e){
      ...
      await store.finishRun(runId,status,nowIso());
      await atomicJson(path.join(out,'failure.json'),{run_id:runId,status,code,discovered:posts.size,publisher_enabled:false});
...
export async function main(argv=process.argv.slice(2)){
  const {values:v,positionals}=parseArgs({args:argv,allowPositionals:true,strict:true,options:{mode:{type:'string'},fixture:{type:'string'},'run-id':{type:'string'},'replay-run-id':{type:'string'},resume:{type:'string'},'max-drafts':{type:'string'},'live-read':{type:'boolean'},config:{type:'string'},'dry-run':{type:'boolean'},apply:{type:'boolean'}}});
  ...
  if(['doctor','status','recover','verify-v1','retention'].includes(command)){
    ...
    if(command==='doctor')return admin.doctor({config,liveRead:v['live-read']});
  ...
  if(!['run','replay'].includes(command))throw new Error('UNKNOWN_COMMAND');
  const controller=new AbortController(),stop=()=>controller.abort(new Error('INTERRUPTED'));
  process.once('SIGINT',stop);process.once('SIGTERM',stop);
  try{return await executeDaily({config,mode:command==='replay'?'REPLAY_REAL_DATA':v.mode||config.mode,fixture:v.fixture?await loadJson(path.resolve(v.fixture)):undefined,replayRunId:command==='replay'?v['run-id']:v['replay-run-id'],resumeRunId:v.resume,maxDrafts:v['max-drafts']===undefined?undefined:Number(v['max-drafts']),signal:controller.signal,onProgress:p=>process.stderr.write(JSON.stringify(p)+'\n')});}
...
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){
  ...
  catch(e){console.error(JSON.stringify({status:'error',code:errorCode(e),publisher_enabled:false}));process.exitCode=1;}
}
